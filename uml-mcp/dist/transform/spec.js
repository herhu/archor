export function transformIRToDesignSpec(ir) {
    const spec = {
        name: "Generated App",
        version: "1.0.0",
        domains: []
    ...
    for (const diagram of ir.diagrams) {
        for (const element of diagram.elements) {
            if (element.type === 'classifier' && element.kind === 'class') {
                const entity = transformClassToEntity(element);
    ...
    for (const seq of ir.diagrams) {
        for (const r of seq.relationships) {
            const rel = r; // Cast to access 'kind' which is property of Message
            if (rel.type === 'message' && (rel.kind === 'sync' || rel.kind === 'async')) {
                ...
                if (!service)
                    service = findService(spec, targetName + "Service");
                if (service) {
...
function findService(spec, name) {
...
function transformClassToEntity(clazz) {
...
function mapType(type) {
    ...
    if (lower === 'string' || lower === 'text')
        return 'string';
    if (lower === 'int' || lower === 'integer' || lower === 'number')
        return 'int'; // Archon uses 'int' or 'number'? sample-spec says 'int'
    if (lower === 'boolean' || lower === 'bool')
        return 'boolean';
    if (lower === 'date' || lower === 'datetime')
        return 'datetime'; // Archon might expect 'date'

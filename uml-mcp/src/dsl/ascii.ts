...
export class AsciiParser {
  ...
  private parseDiagram(tokens: Token[]): any {
    ...
    const peek = (offset = 0) => {
      if (currentTokenIndex + offset >= tokens.length) return null;
    ...
    const fragments: any[] = [];
    const activeFragments: any[] = []; // Stack for fragments
        ...
        try {
            ...
            if (token.type === 'IDENT' && token.value === 'class') {
                 ...
                 if (peek()?.value === 'as') {
                    ...
                    if (aliasToken) clazz.id = aliasToken.value;
                 }
                 ...
                 if (peek()?.value === '{') {
                     ...
                     while (peek() && peek()?.value !== '}') {
                         ...
                         if (!memberToken) break;
                         if (memberToken.type === 'NEWLINE') continue;
                         ...
                         if (memberToken.type === 'IDENT' && peek()?.value === ':') {
                             ...
                             while (peek() && peek()?.type !== 'NEWLINE' && peek()?.value !== '}') { consume(); }
            ...
            // --- Component Parsing ---
                ...
                if (peek()?.value === 'as') {
                    consume();
                     const aliasToken = consume();
                     if (aliasToken) id = aliasToken.value;
                }
                ...
                if (peek()?.value === '{') {
                    ...
                    while (peek() && peek()?.value !== '}') {
                        ...
                        if (!portToken) break;
                        if (portToken.type === 'NEWLINE') continue;
                         ...
                         if (portToken.value === 'provides' || portToken.value === 'requires') {
                             ...
                             if(peek()?.value === ':') { consume(); ifaceId = consume()?.value; }
            ...
            // --- Use Case Parsing ---
                ...
                diagramKind = "usecase";
                const nameToken = consume();
                if(!nameToken) { this.error("Expected name", token); continue; }
            ...
            // --- Sequence Parsing ---
                 ...
                 if (peek()?.value === 'as') { consume(); id = consume()?.value || id; }
                ...
                const fromId = token.type === 'STRING' ? token.value.replace(/\s+/g, '_') : token.value;
                 if (peek()?.type === 'OP' && (peek()?.value.includes('--') || peek()?.value.includes('->') || peek()?.value.includes('..>'))) {
                     ...
                     if (targetToken) {
                         ...
                         let kind = "sync";
                         if (opToken?.value.includes('..>')) type = "dependency";
                         if (opToken?.value === '-->') { type = "message"; kind = "reply"; }
                         if (opToken?.value === '->') { type = "message"; kind = "sync"; }
            ...
            while (peek() && peek()?.type !== 'NEWLINE') consume();
        }
    }

...
async function main() {
    ...
    const client = new Client(
        { name: "GenTestClient", version: "1.0.0" },
    ...
    try {
        ...
        if (fs.existsSync(testOutDir)) {
            ...
            if (files.length > 0) {
                ...
                if (hasRedisFile) console.log("✅ Redis module file created.");
                else console.error("❌ Redis module file MISSING.");

                const pkgJson = fs.readFileSync(path.join(testOutDir, 'package.json'), 'utf-8');
                if (pkgJson.includes("ioredis")) console.log("✅ package.json contains ioredis.");
                else console.error("❌ package.json MISSING ioredis.");

                const appModule = fs.readFileSync(path.join(testOutDir, 'src/app.module.ts'), 'utf-8');
                if (appModule.includes("RedisModule")) console.log("✅ app.module.ts imports RedisModule.");
                else console.error("❌ app.module.ts MISSING RedisModule import.");

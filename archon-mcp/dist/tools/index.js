...
const jsonSchemaToZod = (schema) => {
    return z.object(Object.keys(schema.properties).reduce((acc, key) => {
        acc[key] = z.any(); // Using any for simplicity during migration
        return acc;
    }, {}));
};
export function registerTools(server) {
    server.tool("archon_validate_spec", "Validate an Archon DesignSpec v1 JSON object. Returns validation errors or success.", {
    ...
    server.tool("archon_generate_project", "Generate a full project from a DesignSpec v1. WARNING: Overwrites files in output directory.", {
        spec: z.any().describe("The full DesignSpec JSON object"),
        ...
        await Archon.generateApp(spec, outDir, isDryRun);
        // Verify generation
        if (!isDryRun) {
    ...
    server.tool("archon_get_schema", "Get the formal JSON Schema for Archon DesignSpec v1. Use this to understand the required structure of a spec.", {}, async () => {
        // @ts-ignore
    ...
    server.tool("archon_list_modules", "List available module presets for injection", {}, async () => {
        return {
            content: [{
    ...
    server.tool("archon_launch_demo", "Luxury final step: Install deps, Setup Env, Run Docker, and Expose via Cloudflared.", {
    ...
    server.tool("archon_generate_from_uml", "One-shot conversion: ASCII DSL -> Spec -> Generated Project. Use this for rapid iteration from diagrams.", {
